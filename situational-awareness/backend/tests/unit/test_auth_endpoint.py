...
def test_bootstrap_admin_creates_first_admin_and_returns_token(monkeypatch) -> None:
    ...
    monkeypatch.setattr(auth, "create_access_token", lambda subject, expires_minutes=None, extra=None: f"token::{subject}::{extra['role']}")
    ...
    assert response.status_code == 200
    assert response.json() == {
        "access_token": "token::user-1::admin",
        "token_type": "bearer",
    }
...
def test_login_accepts_email_and_returns_token(monkeypatch) -> None:
    ...
    assert response.status_code == 200
    assert response.json() == {
        "access_token": "token::user-existing::admin",
        "token_type": "bearer",
    }
